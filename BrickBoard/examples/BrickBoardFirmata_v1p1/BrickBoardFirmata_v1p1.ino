 ...
 *  added by h2in(h2in@iamamaker.kr)
 */
		case _BRICKBOARD_LED01_:
			if (argv[0] != 0xFF) {
				BrickBoard_Ports	led01(argv[0], MODE0);
				if (argv[1] == 0x01) {
		...
		case _BRICKBOARD_SW01_:
			if (argv[0] != 0xFF) {
				BrickBoard_Ports	sw01(argv[0], MODE0);
				Firmata.write(START_SYSEX);
				Firmata.write(STRING_DATA);
				if (argv[1] == 0x01) {
					Serial.println(sw01.readDigital(S1));
				}
				else if (argv[1] == 0x02) {
		...
		case _BRICKBOARD_MT01_:
			if (argv[0] != 0xFF) {
				BrickBoard_Ports	mt01(argv[0], MODE0);
				mt01.writeDigital(S1, (argv[1] & 0x01));
				mt01.writeDigital(S2, ((argv[1] & 0x02) >> 1));
			}
			else {}
		break;

		case _BRICKBOARD_PZ01_:
			static int preFreq = 0, prePort = 0;
			if (argv[0] != 0xFF) {
				BrickBoard_Ports	pz01(argv[0], MODE0);
				...
				if (argv[1] == 0x01) {
					...
					if ((prePort != 0) && (prePort != argv[0])) {
						BrickBoard_Ports pz01_pre(prePort, MODE0);
						preFreq = 0;
						noTone(pz01_pre.getPin(S1));
					}
					else {}
					prePort = argv[0];
					...
					if (freq != preFreq) {
						preFreq = freq;
						tone(pz01.getPin(S1), freq);
					}
					else {}
				}
				else {
					preFreq = 0;
					noTone(pz01.getPin(S1));
				}
			}
			else {}
		break;

		case _BRICKBOARD_VR01_:
			if (argv[0] != 0xFF) {
				BrickBoard_Ports vr01(argv[0], MODE0);
				Firmata.write(START_SYSEX);
				Firmata.write(STRING_DATA);
		...
		case _BRICKBOARD_IR01_:
			if (argv[0] != 0xFF) {
				BrickBoard_Ports ir01(argv[0], MODE0);
				if (argv[1] == 0x01) {
					pinMode(ir01.getPin(S1), INPUT);
					ir01.writeDigital(S2, argv[2]);
				}
				else {
					Firmata.write(START_SYSEX);
					Firmata.write(STRING_DATA);
		...
		case _BRICKBOARD_MP01_:
			if (argv[0] != 0xFF) {
				BrickBoard_MP01	mp01(argv[0], MODE0);
				switch (argv[1]) {
					case 0x0: //getPin
						Firmata.write(START_SYSEX);
						Firmata.write(STRING_DATA);
  ...
  for (byte i = 0; i < TOTAL_PINS; i++) {
    ...
    if (IS_PIN_ANALOG(i)) {
      ...
      setPinModeCallback(i, PIN_MODE_ANALOG);
			...
			/* modified by h2in(h2in@iamamaker.kr) */
